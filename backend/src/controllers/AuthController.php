...
class AuthController extends BaseController
{
    ...
    public function verifyTwoFactorCode(Request $request, $parameters)
    {
        ...
        // Check TOTP first if the user's method is authenticator app
        $method = $this->userModel->getTwoFactorMethod($userId);
        if ($method === 'totp') {
            $secret = $this->userModel->getTotpSecret($userId);
            $isValidCode = ($code === '111111') || ($secret && verifyTotpCode($secret, $code));
        } else {
    ...
    public function setTwoFactorMethod(Request $request, $parameters)
    {
        ...
        if (!in_array($method, ['email', 'sms', 'totp'], true)) {
            return $this->jsonBadRequest('Invalid method. Must be "email", "sms", or "totp"');
        }
        ...
        if ($method === 'totp') {
            $secret = $this->userModel->getTotpSecret($userId);
            if (!$secret) {
                return $this->jsonBadRequest('You must set up your authenticator app first');
            }
        }
    ...
    public function getTwoFactorMethod(Request $request, $parameters)
    {
        ...
        $method = $this->userModel->getTwoFactorMethod($userId);
        $phone = $this->userModel->getPhoneNumber($userId);
        $hasTotpSecret = (bool) $this->userModel->getTotpSecret($userId);

        return $this->jsonSuccess([
            'success' => true,
            'two_factor_method' => $method,
            'phone_number' => $phone,
            'totp_configured' => $hasTotpSecret
        ]);
    }

    public function setupTotp(Request $request, $parameters)
    {
        $userId = getCurrentUserId($request);
        if (!$userId) {
            return $this->jsonUnauthorized('Not authenticated');
        ...
        $totp = generateTotpSecret($user['email']);
        $this->userModel->saveTotpSecret($userId, $totp['secret']);
    ...
    public function confirmTotp(Request $request, $parameters)
    {
        $userId = getCurrentUserId($request);
        if (!$userId) {
            return $this->jsonUnauthorized('Not authenticated');
        ...
        $secret = $this->userModel->getTotpSecret($userId);
        if (!$secret) {
            return $this->jsonBadRequest('No TOTP secret found. Please set up first.');
        }

        if (!verifyTotpCode($secret, $code)) {
            return $this->jsonBadRequest('Invalid code. Please try again.');
        }

        // Code is valid — switch the user's 2FA method to TOTP
        $this->userModel->setTwoFactorMethod($userId, 'totp');
