...
### HTTP request
    const res = await fetch('/api/user/login', {-> api point login stuff, returns id (David could change it)
      method: 'POST',	-> post means write into it (Get would mean get the data)
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ usernameOrEmail: username.trim(), password })}) -> the parameter the backend requires
