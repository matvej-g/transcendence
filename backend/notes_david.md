...
- directives
	- key value pairs
- context
	- blocks containing directives

- reverse proxy
	- server side proxy
	- conceals the identity of the backend application services
	- acts in place of the services

- proxy_pass Directive
	- proxy_pass <destination>
	- moves an incoming request to a replacement destination at the backend
	- only used inside server and location context
		- location / {
			proxy_pass http://10.1.1.4:9000/api1;
		}
	
- capture incoming data to be able to dinstinguish it from nginx starting point
	- proxy_set_header
